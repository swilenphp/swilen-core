...
it('Get the content of the Buffer file generated by the BinaryFileResponse instance', function () {
    ...
    expectt($response->headers->get('Content-Type'))->toMatch('/text\/markdown+/');

    expectt($response->getStatusCode())->toBe(HttpStatus::OK->value);
    expectt($content)->toBe('Testing Markdown');
    expectt($response->getFile())->toBeTruthy();
    expectt($response->getFile())->toBeInstanceOf(File::class);
});

it('Raise error when BinaryFileResponse instance contains a file path that does not exist', function () {
    expectt(new BinaryFileResponse(__DIR__ . '/../__fixtures__/nothing.md'))->toBeResource();
})->throws(FileNotFoundException::class);

it('BinaryFileResponse as attachment file', function () {
    $response = new BinaryFileResponse(getReadableFileStub(), HttpStatus::OK->value, [], ResponseHeaderHunt::DISPOSITION_ATTACHMENT);

    expectt($response->hasHeader('Content-Disposition'))->toBeTrue();

    $response->updateFilename('test-foo.txt');
    expectt($response->headers->get('Content-Disposition'))->toBe('attachment; filename="test-foo.txt"');
});
